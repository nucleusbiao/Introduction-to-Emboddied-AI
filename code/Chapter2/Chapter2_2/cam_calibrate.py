...
def calibrate_camera_from_images(images_folder='cam_board_pic'):
    """从保存的图像文件夹进行相机标定"""
    # 设置棋盘格尺寸
    ...
    if len(image_files) == 0:
        print(f"错误：在文件夹 '{images_folder}' 中没有找到图像文件！")
        return None, None

    print(f"\n从文件夹 '{images_folder}' 中找到 {len(image_files)} 张图像")
    print("开始处理图像并检测角点...")
    ...
    print(f"\n成功处理 {valid_images_count}/{len(image_files)} 张图像")

    if valid_images_count < 10:
        print(f"警告：有效图像数量不足！至少需要 10 张，当前只有 {valid_images_count} 张")
        return None, None

    print(f"\n开始张正友标定法计算，使用 {valid_images_count} 张图像...")
...
def calibrate_camera():
    ...
    while True:
        ...
        display_frame = cv2_add_chinese_text(display_frame, f"保存到: {save_folder}", (10, 90), (0, 255, 0), 20)
        display_frame = cv2_add_chinese_text(display_frame, "按 'q' 退出", (10, 120), (0, 255, 0), 24)
        display_frame = cv2_add_chinese_text(display_frame, f"需要至少 {min_images} 张", (10, 150), (255, 255, 0), 20)
        ...
        if key == ord('s'):
            ...
            if ret_corners and corners_refined is not None:
                try:
                    ...
                    filename = f"calibration_{saved_count:04d}_{timestamp}.jpg"
                    ...
                    temp_frame = cv2_add_chinese_text(temp_frame, "图像已保存!", (10, 180), (0, 0, 255), 24)
    ...
    if len(objpoints) < min_images:
        ...
        # 但图片已经保存到文件夹中，可以后续使用
        print(f"原始图片已保存到 '{save_folder}' 文件夹中，可以后续使用选项2进行标定")
        return None, None
...
if __name__ == "__main__":
    print("张正友相机标定法实现")
    print("1. 进行新的标定（保存原始图片到 cam_board_pic 文件夹）")
    print("2. 从 cam_board_pic 文件夹中的图片进行标定")
    print("3. 加载已有标定结果")
    ...
    if choice == "1":
        ...
        camera_matrix, dist_coeffs = calibrate_camera_from_images('cam_board_pic')
        if camera_matrix is not None:
            test_calibration(camera_matrix, dist_coeffs)
    elif choice == "3":
        try:
